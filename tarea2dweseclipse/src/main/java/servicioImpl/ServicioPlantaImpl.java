...
public class ServicioPlantaImpl implements ServicioPlanta{
    ...
    public int modificar(Planta planta) {
        if (planta == null) {
            throw new IllegalArgumentException("La planta no puede ser null.");
        }
        if (planta.getCodigo() == null || planta.getCodigo().trim().isEmpty()) {
            throw new IllegalArgumentException("El código de la planta no puede ser null o vacío.");
        }
